...
Matrix3x3 parse_transformation(const json &node) {
    ...
    for (auto it = transform_it->begin(); it != transform_it->end(); it++) {
        if (auto scale_it = it->find("scale"); scale_it != it->end()) {
            ...
            Matrix3x3 mRotate;
            mRotate(0,0) = cos(angle*c_PI/Real(180));
            mRotate(0,1) = -sin(angle*c_PI/Real(180));
            mRotate(1,0) = sin(angle*c_PI/Real(180));
            mRotate(1,1) = cos(angle*c_PI/Real(180));
            mRotate(2,2) = 1;
            F = mRotate * F;
            ...
            Matrix3x3 mTranslate;
            mTranslate(0,0) = 1;
            mTranslate(1,1) = 1;
            mTranslate(0,2) = translate.x;
            mTranslate(1,2) = translate.y;
            mTranslate(2,2) = 1;
            F = mTranslate * F;
